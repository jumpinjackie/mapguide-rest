...
class MgFeatureXmlRestAdapterDocumentor extends MgFeatureRestAdapterDocumentor {
    protected function GetAdditionalParameters(IAppServices $app, $bSingle, $method) {
...
class MgFeatureXmlSessionIDExtractor extends MgSessionIDExtractor {
     ...
     */
    public function TryGetSessionId(IAppServices $app, $method) {
        if ($method == "POST" || $method == "PUT") {
            $doc = new DOMDocument();
            $doc->loadXML($app->GetRequestBody());
        ...
        try {
            ...
            if ($single === true) {
                ...
                if ($idProps->GetCount() != 1) {
                    $this->app->Halt(400, $this->app->GetLocalizedText("E_CANNOT_APPLY_UPDATE_CANNOT_UNIQUELY_IDENTIFY", $this->featureId, $idProps->GetCount()), "text/plain", false);
                } else {
