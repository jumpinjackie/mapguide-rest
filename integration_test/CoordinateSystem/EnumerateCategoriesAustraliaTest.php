...
class EnumerateCategoriesAustraliaTest extends ServiceTest {
    public function __testOperation($extension, $mimeType) {
        
        //With raw credentials
        $resp = $this->apiTestAnon("/coordsys/category.$extension/Australia", "GET", array());
        $this->assertStatusCodeIs(200, $resp);
        $this->assertMimeType($mimeType, $resp);
        $this->assertContentKind($resp, $extension);

        $resp = $this->apiTestAdmin("/coordsys/category.$extension/Australia", "GET", array());
        $this->assertStatusCodeIs(200, $resp);
        $this->assertMimeType($mimeType, $resp);
        $this->assertContentKind($resp, $extension);

        //With session id
        $resp = $this->apiTest("/coordsys/category.$extension/Australia", "GET", array("session" => $this->anonymousSessionId));
        $this->assertStatusCodeIs(200, $resp);
        $this->assertMimeType($mimeType, $resp);
        $this->assertContentKind($resp, $extension);

        $resp = $this->apiTest("/coordsys/category.$extension/Australia", "GET", array("session" => $this->adminSessionId));
        $this->assertStatusCodeIs(200, $resp);
        $this->assertMimeType($mimeType, $resp);
        $this->assertContentKind($resp, $extension);
    }
    public function testBadRequests() {
        $resp = $this->apiTestAnon("/coordsys/category.sadgdsfd/Australia", "GET", array());
        $this->assertStatusCodeIs(400, $resp);
        $this->assertMimeType(Configuration::MIME_HTML, $resp);

        $resp = $this->apiTestAdmin("/coordsys/category.sadgdsfd/Australia", "GET", array());
        $this->assertStatusCodeIs(400, $resp);
        $this->assertMimeType(Configuration::MIME_HTML, $resp);
        ...
        foreach ($extensions as $ext) {
            ...
            $resp = $this->apiTest("/coordsys/category.$extension/Australia", "GET", array());
