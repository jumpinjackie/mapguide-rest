...
class GetSpatialContextsApiTest extends ServiceTest {
    ...
    private function __testCommon($resPart) {
        $resp = $this->apiTestAnon("$resPart/spatialcontexts.sdigud", "GET", null);
        $this->assertStatusCodeIs(400, $resp);
        $this->assertMimeType(Configuration::MIME_HTML, $resp);

        $resp = $this->apiTestAdmin("$resPart/spatialcontexts.sdigud", "GET", null);
        $this->assertStatusCodeIs(400, $resp);
        $this->assertMimeType(Configuration::MIME_HTML, $resp);
    }
    private function __testBase($resPart, $extension, $mimeType, $bTestUnauth) {
        if ($bTestUnauth) {
            $resp = $this->apiTest("$resPart/spatialcontexts.$extension", "GET", null);
            $this->assertStatusCodeIs(401, $resp);
            $this->assertMimeType($mimeType, $resp);
        }
        //Raw credentials
        $resp = $this->apiTestAnon("$resPart/spatialcontexts.$extension", "GET", null);
        $this->assertStatusCodeIs(200, $resp);
        $this->assertMimeType($mimeType, $resp);
        $this->assertContentKind($resp, $extension);

        $resp = $this->apiTestAdmin("$resPart/spatialcontexts.$extension", "GET", null);
        $this->assertStatusCodeIs(200, $resp);
        $this->assertMimeType($mimeType, $resp);
        $this->assertContentKind($resp, $extension);

        //Session ID
        $resp = $this->apiTest("$resPart/spatialcontexts.$extension", "GET", array("session" => $this->anonymousSessionId));
        $this->assertStatusCodeIs(200, $resp);
        $this->assertMimeType($mimeType, $resp);
        $this->assertContentKind($resp, $extension);

        $resp = $this->apiTest("$resPart/spatialcontexts.$extension", "GET", array("session" => $this->adminSessionId));
        $this->assertStatusCodeIs(200, $resp);
        $this->assertMimeType($mimeType, $resp);
        $this->assertContentKind($resp, $extension);
    }
