...
try
{
    ...
    $testId = new MgResourceIdentifier("Library://Samples/Sheboygan/Layouts/SheboyganPhp.WebLayout");

    if (!$resSvc->ResourceExists($testId)) {
        $bsData = new MgByteSource(dirname(__FILE__)."/data/Sheboygan.mgp");
        $brData = $bsData->GetReader();
        $resSvc->ApplyResourcePackage($brData);

        $featSvc = $siteConn->CreateService(MgServiceType::FeatureService);

        //Commercial sample
        $res1 = new MgResourceIdentifier("Library://Samples/Sheboygan/MapsTiled/SheboyganNoWatermark.MapDefinition");
        $bs1 = new MgByteSource(dirname(__FILE__)."/data/SheboyganNoWatermark.MapDefinition.xml");
        $br1 = $bs1->GetReader();
        ...
        $bs2 = new MgByteSource(dirname(__FILE__)."/data/SheboyganMixed.MapDefinition.xml");
        $br2 = $bs2->GetReader();
        ...
        $bs3 = new MgByteSource(dirname(__FILE__)."/data/RESTWebLayout.mgp");
        $br3 = $bs3->GetReader();
